...
void SelectData(OdomDataList &odoDatas, CamDataList &camDatas, std::vector<data_selection::SyncData> &sync_result) {
  ...
  for (int i = 0; i < size; ++i) {
    if (fabs(odoDatas[i].v_left) < 1e-3 || fabs(odoDatas[i].v_right) < 1e-3) continue;
    // the sign is opposite
    if ((odoDatas[i].v_right - odoDatas[i].v_left) * camDatas[i].delta_theta < 0.0)  // rL == rR
    {
      nFlagDiff++;
      continue;
    }
...
void CamOdoAlign(OdomDataList &odoDatas, CamDataList &camDatas, SyncDataList &sync_result) {
  ...
  for (int i = 3; unsigned(i) < camDatas.size() - 3; ++i) {
    if (std::isnan(camDatas[i].delta_theta)) continue;
    ...
    if (odo_end_id - odo_start_id == 0) continue;
    ...
    if (camDatas[i].end_t > odo_end.time) {
      float alpha =
          (camDatas[i].end_t - odoDatas[odo_end_id].time) / (odoDatas[odo_end_id + 1].time - odoDatas[odo_end_id].time);
      alpha = fabs(alpha);
      ...
      float alpha =
          (odoDatas[odo_end_id].time - camDatas[i].end_t) / (odoDatas[odo_end_id].time - odoDatas[odo_end_id - 1].time);
      alpha = fabs(alpha);
      v_left_end = (1 - alpha) * odoDatas[odo_end_id].v_left + alpha * odoDatas[odo_end_id - 1].v_left;
      v_right_end = (1 - alpha) * odoDatas[odo_end_id].v_right + alpha * odoDatas[odo_end_id - 1].v_right;
    } else {
    ...
    if (odo_end_id - odo_start_id > 1) {
      ...
      for (int j = odo_start_id; j < odo_end_id; j++) {
        ...
        dis_right_sum +=
            (odoDatas[j + 1].time - odoDatas[j].time) * (odoDatas[j + 1].v_right + odoDatas[j].v_right) / 2.0;
      }
      ...
      sync_tmp.scan_match_results[2] = camDatas[i].delta_theta;

      sync_tmp.tcl_cam = -camDatas[i].Rcl * camDatas[i].tlc;
      sync_tmp.qcl_cam = Eigen::Quaterniond(camDatas[i].Rcl);
      sync_tmp.angle = -camDatas[i].delta_theta;  // should be cl
      sync_tmp.axis = camDatas[i].axis;
      sync_tmp.start_time = camDatas[i].start_t;
...
void StartPosAlign(OdomDataList &odoDatas, CamDataList &camDatas) {
  ...
  if (t0_cam > t0_odo) {
    ...
    for (int i = 0; i < odo_aligned; ++i) odoDatas.erase(odoDatas.begin());

    std::cout << std::fixed << "aligned start position1: " << camDatas[0].start_t << "  " << odoDatas[0].time
              << std::endl;
    ...
    for (int i = 0; i < cam_aligned; ++i) camDatas.erase(camDatas.begin());

    std::cout << std::fixed << "aligned start position2 : " << camDatas[0].start_t << "  " << odoDatas[0].time
              << std::endl;
  }
}
}  // namespace data_selection
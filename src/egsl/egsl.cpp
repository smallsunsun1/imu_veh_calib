...
int its_context(val v) { return v.cid; }

int its_var_index(val v) { return v.index; }
...
void egsl_print_stats() {
  ...
  for (c = 0; c <= max_cid && c < MAX_CONTEXTS; c++) {
    ...
    fprintf(stderr, "egsl: context #%d allocations: %d active: %d name: '%s' \n", c, egsl_contexts[c].nallocated,
            egsl_contexts[c].nvars, egsl_contexts[c].name);
  }
}
...
void egsl_expect_size(val v, size_t rows, size_t cols) {
  ...
  if (bad) {
    fprintf(stderr, "Matrix size is %d,%d while I expect %d,%d", (int)m->rows(), (int)m->cols(), (int)rows, (int)cols);
...
void egsl_print(const char* str, val v) {
  ...
  for (i = 0; i < m->rows(); i++) {
    ...
    for (j = 0; j < m->cols(); j++) fprintf(stderr, "%f ", gsl_matrix_get(m, i, j));
...
double egsl_atv(val v1, size_t i) { return *egsl_atmp(v1, i, 0); }

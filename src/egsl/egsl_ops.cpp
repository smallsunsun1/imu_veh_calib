...
val egsl_sub(val v1, val v2) { return egsl_sum(v1, egsl_scale(-1.0, v2)); }
...
val egsl_compose_col(val v1, val v2) {
  ...
  int i, j;
  for (j = 0; j < m1->cols(); j++) {
    for (i = 0; i < m1->rows(); i++) gsl_matrix_set(m3, i, j, gsl_matrix_get(m1, i, j));

    for (i = 0; i < m2->rows(); i++) gsl_matrix_set(m3, m1->rows() + i, j, gsl_matrix_get(m2, i, j));
  }
  return v3;
}
...
val egsl_compose_row(val v1, val v2) {
  ...
  int i, j;
  for (i = 0; i < m1->rows(); i++) {
    for (j = 0; j < m1->cols(); j++) gsl_matrix_set(m3, i, j, gsl_matrix_get(m1, i, j));

    for (j = 0; j < m2->cols(); j++) gsl_matrix_set(m3, i, m1->cols() + j, gsl_matrix_get(m2, i, j));
  }
  return v3;
}

...
void filter_orientation(double theta0, double rho0, size_t n, const std::vector<double>& thetas,
                        const std::vector<double>& rhos, double* alpha, double* cov0_alpha);
...
void ld_compute_orientation(LDP ld, int size_neighbourhood, double sigma) {
  ...
  for (i = 0; i < ld->nrays; i++) {
    ...
    filter_orientation(ld->theta[i], ld->readings[i], num_neighbours, thetas, readings, &alpha, &cov0_alpha);
#ifndef WINDOWS
    if (std::isnan(alpha)) {
#else
    if (_isnan(alpha)) {
...
void filter_orientation(double theta0, double rho0, size_t n, const std::vector<double>& thetas,
  ...
  *alpha = theta0 - atan(f1 / rho0);
  ...
  if (cos(*alpha) * cos(theta0) + sin(*alpha) * sin(theta0) > 0) *alpha = *alpha + M_PI;

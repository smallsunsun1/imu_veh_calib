...
int minmax(int from, int to, int x) { return (std::max)((std::min)(x, to), from); }

void possible_interval(const double *p_i_w, LDP ld, double max_angular_correction_deg, double max_linear_correction,
                       int *from, int *to, int *start_cell) {
  double angle_res = (ld->max_theta - ld->min_theta) / ld->nrays;

  /* Delta for the angle */
  double delta = fabs(deg2rad(max_angular_correction_deg)) + fabs(atan(max_linear_correction / norm_d(p_i_w)));
  ...
  *start_cell = (int)((start_theta - ld->min_theta) / (ld->max_theta - ld->min_theta) * ld->nrays);
...
double distance_d(const double a[2], const double b[2]) { return sqrt(distance_squared_d(a, b)); }

int is_nan(double v) { return v == v ? 0 : 1; }

int any_nan(const double *d, int n) {
  int i;
  for (i = 0; i < n; i++)
    if (is_nan(d[i])) return 1;
  return 0;
}
...
void projection_on_segment_d(const double a[2], const double b[2], const double x[2], double proj[2]) {
  ...
  if ((proj[0] - a[0]) * (proj[0] - b[0]) + (proj[1] - a[1]) * (proj[1] - b[1]) < 0) {
...
double dot_d(const double p[2], const double q[2]) { return p[0] * q[0] + p[1] * q[1]; }
...
int segment_ray_tracing(const double p0[2], const double p1[2], const double eye[2], double direction, double *range) {
  *range = std::numeric_limits<double>::quiet_NaN();
  ...
  if (dist_to_midpoint > seg_size / 2) return 0;
...
const char *friendly_pose(const double *pose) {
  sprintf(tmp_buf, "(%4.2f mm, %4.2f mm, %4.4f deg)", 1000 * pose[0], 1000 * pose[1], rad2deg(pose[2]));
  return tmp_buf;
}

...
void ld_alloc(LDP ld, int nrays) {
  ...
  ld->corr = (struct correspondence*)malloc(sizeof(struct correspondence) * nrays);
  ...
  for (i = 0; i < 3; i++) {
    ld->odometry[i] = ld->estimate[i] = ld->true_pose[i] = std::numeric_limits<double>::quiet_NaN();
  }
  ...
  for (i = 0; i < nrays; i++) {
    ld->points[i].p[0] = ld->points[i].p[1] = ld->points[i].rho = ld->points[i].phi =

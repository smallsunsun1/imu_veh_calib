...
void sm_debug_write(int flag) { sm_debug_write_flag = flag; }
...
void check_for_xterm_color() {
  ...
  xterm_color_available = !strcmp(term, "xterm-color") || !strcmp(term, "xterm") || !strcmp(term, "rxvt");
...
void sm_error(const char *msg, ...) {
  ...
  if (xterm_color_available) fprintf(stderr, XTERM_ERROR);

  if (sm_program_name) fprintf(stderr, "%s: ", sm_program_name);

  sm_write_context();

  if (!xterm_color_available) fputs(":err: ", stderr);

  va_list ap;
  va_start(ap, msg);
  vfprintf(stderr, msg, ap);

  if (xterm_color_available) fprintf(stderr, XTERM_COLOR_RESET);
}
...
void sm_info(const char *msg, ...) {
  ...
  if (!xterm_color_available) fputs(":inf: ", stderr);
...
void sm_debug(const char *msg, ...) {
  ...
  if (xterm_color_available) fprintf(stderr, XTERM_DEBUG);

  if (sm_program_name) fprintf(stderr, "%s: ", sm_program_name);

  sm_write_context();

  if (!xterm_color_available) fputs(":dbg: ", stderr);
...
void sm_write_context() {
  for (int i = 0; i < sm_log_context; i++) fprintf(stderr, "   ");
}

...
bool SolveQyx::estimateRyx(std::vector<data_selection::SyncData> sync_result, Eigen::Matrix3d &Ryx) {
  ...
  for (size_t i = 0; i < sync_result.size(); ++i) {
    ...
    M_tmp << 0, -1 - axis[2], axis[1], -axis[0], axis[2] + 1, 0, -axis[0], -axis[1], -axis[1], axis[0], 0, 1 - axis[2],
...
void SolveQyx::refineEstimate(Eigen::Matrix4d &Trc, double scale, const std::vector<Eigen::Quaterniond> &quats_odo,
  ...
  for (size_t i = 0; i < quats_odo.size(); ++i) {
    ceres::CostFunction *costfunction = new ceres::AutoDiffCostFunction<CameraOdomErr, 6, 4, 3>(new CameraOdomErr(

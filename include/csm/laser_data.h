...
struct laser_data {
  ...
  int *restrict cluster;  // Cluster to which point i belongs. This is used for computing the orientation (at the moment
                          // a really dumb algorithm is used for clustering). If cluster[i] == -1 , the point does not
  ...
  int *restrict up_bigger, *restrict up_smaller, *restrict down_bigger, *restrict down_smaller;
};
...
void possible_interval(const double *p_i_w, LDP laser_sens, double max_angular_correction_deg,
